...
app.get("/",(req,res)=>{
    res.sendFile(__dirname+"/public/index.html");//this wll host ffe and be on same server no need of cors
})
...
app.get("/me",(req,res)=>{//authanticated ep, means you have to be signed in to acess this
    ...
    found=users.find(user=>user.username===username); //now i can call db for data of the user, can do this in aut for better auth, reed auth comments
    if(found){
        res.json({
...
// token will also give the same payload as token generated but when verified it will result in an error
...
// Or can use someone cookies to login as them
