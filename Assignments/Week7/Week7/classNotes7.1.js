...
app.post("/signup",async (req,res)=>{
    ...
    let salt=await bcrypt.genSalt(saltingRounds).catch((err)=>{//this generates a random salt(string) to add to passowrd before hashing it
        console.log("There was an error while salting!! Err Status: "+err);
    });
    let hashedPassword= await bcrypt.hash(password,salt).catch((err)=>{//convertes the salted password to a random fixed-length string that can be converted back to the original one
        console.log("There was an error while Hashing Password!! Err Status: "+err);
    })
