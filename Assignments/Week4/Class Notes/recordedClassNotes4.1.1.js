...
// This http server isn't open for everyone yet!! its accessible for everyone on same network, so i can use my phone 
...
//     //if theres any error while executing express will automatically send status code 500
// })
...
// app.listen(5500);//i am listening infinitely, whenerver req comes to / or /asd endpoint(routs) ill repond with something
...
//popular way to take input in this request method is: query
...
app.delete("/",(req,res)=>{
    //if theres no unhealthy kidney we should return status code 411-for wrong input(basically the best practice is ki we should be checking all edge cases and respond accordongly)
    let noOfUnhealthyKidneys=users[0].kidneys.filter(kidney => kidney.healthy===false).length;
