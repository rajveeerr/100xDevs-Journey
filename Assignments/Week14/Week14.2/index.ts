...
interface User{// here in interfaces i can put real values too instead of types, that will mean ki whatever var is using this type has to have that value inside
    name: string,
...
interface Employee{
    ...
    greet2(phrase:string): string, // another way of writing functions, this says that the greet is a method tied to object,
    // toh in the implementation there will be two keys and a method(without any key) in the type, treated as method tied 
    // to an object
}
...
let manager:Employee={
    ...
    // } 
    // this keyword works here because the method is tied to the
    // object and now has acess to all the member variables as the function itself became member of the object also normal
    // objects can have this keyword, for arrow fn they dont have 'this' they inherit 'this' from parent scope, thats why
    // we dont use them in the eventlisteners
}
...
let manager2:Employee={
    ...
    // variables, to make it work without using 'this' keyword we'll have to use the variable just created and use it  - solved
    // greet2: function(phrase){return phrase+this.name} // actually 'this' works here too, in above line it wasnt working
    // bc there was an arrow fn being created, and arrow fns dont have acess to this 'this' their 'this' is equivalent to
    // window
}
...
class Shape{
    name:string // these are public members, we can create protected and public members too just like cpp
    width:number;
    height:number;
    constructor(){
        this.name=""
...
// encaptulation means hiding internal details by making it private and providing public methods to interact with these private members
...
abstract class People{
    ...
    abstract age:number;
    // constructor(name:string,age:number){ //no need to call constructor for initializing abstract variables
    //     this.name=name
    //     this.age=age
    // }
...
class Student extends People{
    // name="Some";//can initialise name here too
    name: string;
    age:number;
...
// one interface can extend another interface, using this property intersection can be performed using interfaces
...
let Athlete:User=proof//some way to get around of the typechecks, i added one more field than what was permitted in User
...
type UsersAdmin= Users&Admin //now this resultant type will have to have all the field of the above two types, for non-primitive datatypes
...
function sayHi(person:UserorAdmin): UserorAdmin{
    ...
    if ("country" in person){// this clearly tells compiler if type Admin is passed at runtime do this, called type guard
        // in operator checks if the specified property exist in the object(and prototype chain), another way of doing
        // this would be typeof="string" if passed type would be number|string
...
function isAdmin(person: UserorAdmin): person is Admin{
    ...
    return (person as Admin).country!==undefined; // as is type assertion, tells compiler to take type as specified
} 
// usage
...
// properties(if all peoperties of type 1 isnt present it will not satisfy type 1 and same can be said for type 2, thats
// why all properties are supposed to be present)

// ANOTHER WAY OF SEEING THIS:-

// the name intersection(&) feels quite opposite to what it does like intersection of two types gives a type with all the 
// properties of those types(wiz union), but actually it does what it says, ts types can be thought of as set of values
// ts has open types that means a type can have infinite set of all the values that must contan the types
...
// a and b and that is what taken as intersection, 
// but my question: is tharah to there can be multiple values, that will intersect like a set that contains value from both 
// the types and some random values that are intersecting then why arent they in intersection -> because we are taking 
// atleast minimum intersecting points
...
// Enums in ts is same as cpp, basically with enums we can created named constants, this will make program more readable
// for a lot of usecases, by giving meaningful names to constants like 1,2,3
