...
x="daadsads"; //this could have resulted in an error, if stype string had not been used
console.log(x);
// if compilation/build is not succesfull, a index.js fill will be spit out regardless(this setting can be changed in 
...
// now the type of post and data parameter is same, it is being repeated, to fix this we can define a custom 
...
type numberOrString=number|string //this type is union of number and string, interface has no equals
...
function delayedFn(func:()=>void|string):void{
  setTimeout(()=>{
    ...
    if(typeof(result)==="string"){
      return result//this sould have resulted in an error dunno why not here-> no why would this return in error, its the return of settimeout fn , not the delayed function
    }
  },1000)
